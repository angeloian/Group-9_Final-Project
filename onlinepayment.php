...
if(isset($_POST['online_payment'])){
  ...
  if($cart_total == 0){
      ...
      if(mysqli_num_rows($cart_query) > 0){
        ...
        mysqli_query($conn, "INSERT INTO `online_payment` (user_id, name, contact_number, email, method, address, total_products, total_price, note, delivery_charges, placed_on, time, tracking_number, reference_number, order_type) VALUES('$user_id', '$name', '$contact_number', '$email', '$method', '$address', '$total_products', '$cart_total', '$note', '$delivery_charges', '$placed_on','$time','$tracking_number','$reference_number','$order_type')") or die('query failed');
